...
// Kiểu dữ liệu state cho form
interface StationFormData {
  name: string;
  location: string;
  descriptions: string;
  phone: string;
  // image và wallpaper có thể là File mới (để upload) hoặc là URL cũ (string)
...
interface StationFormModalProps {
  visible: boolean;
  onClose: () => void;
  onSave: (data: Station) => void; // Hàm gọi API/Redux để lưu
  initialData?: Station | null; // Dữ liệu khi ở chế độ Sửa
...
const StationFormModal: React.FC<StationFormModalProps> = ({
  ...
  useEffect(() => {
    ...
    // LƯU Ý: Đã xóa lệnh form.resetFields() dư thừa ở đây để fix lỗi Sửa
    ...
    >
      ...
      >
        ...
        <Row gutter={16} className="mt-4">
          <Col span={12}>
            <Form.Item label="Ảnh Chính (Card)">
              <Input
                type="file"
                accept="image/*"
